 ...
 * @param x 削除する値
 */
function deleteb(root, x) {
    var parent = root; // 削除対象ノードpの親ノード
    var p = root; // 削除対象ノード
    ...
    if (p.l === null || p.r === null) {
        var q = // pの残された子ノード　または null
         void 0; // pの残された子ノード　または null
        if (p.r === null) {
            q = p.l;
        }
        else {
    ...
    //この場合、削除対象ノードの次に大きいノードを張り付ける
        ...
        var q = parent.r; //parentの右部分木の最小値ノード
        var g = p; // qの親ノード
        // qがpの右部分木で最小値を見つける
        // q-> l がnullになるまで左へ進む
...
// 使用例
var root = new Vertex(10); // 根ノードを10で作成
insert(root, 5);
insert(root, 15);
insert(root, 3);
insert(root, 7);
