...
class DataStore {
    private products = new Map<string, Product>([
        [
            "PROD-PC",
            new Product("PROD-PC", "Gaming PC", 1200, "premium", "SUPP-COMP", [
                "PROD-CPU",
                "PROD-MEM",
                "PROD-SSD",
            ], ["CN", "RU"]),
...
class DataLoader<K, V> {
        ...
        return new Promise((resolve) => {
            const checkAll = () => {
                setTimeout(() => {
                    ...
                    if (
                        keys.every((k) =>
                            this.cache.has(k) || this.cache.get(k) === null
                        )
                    ) {
                        resolve(results);
                    } else {
    ...
    private async executeBatch(): Promise<void> {
        ...
        results.forEach((value, idx) => {
            if (value !== null) {
                this.cache.set(keys[idx], value);
            } else {
                this.cache.set(keys[idx], null as any);
            }
        });
    }
...
class OrderQueryEngine {
    ...
    > {
        ...
        const componentSupplierIds = product.componentIds.map((id) => {
    ...
    > {
        ...
        const tierDiscount = subtotal *
            tierDiscounts[orderDetails.customer.tier];
    ...
    private async calculateComponentDiscount(
        orderDetails: any,
    ): Promise<number> {
        let discount = 0;
